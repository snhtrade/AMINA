...
// Member Info
function apms_member($mb_id, $lvl='yes') {
...
function apms_video_id($url, $vid, $type) {
	...
	if($type == "tvcast"){ 
		preg_match('/\<meta property=\"og\:video:url\"([^\<\>])*\>/i', $output, $video);
		if($video) {
			$video = apms_query($video[0]);
	...
	} else if($type == "daum" || $type == "kakao") {
		preg_match('/\<meta property=\"og\:url\"([^\<\>])*\>/i', $output, $video);
		if($video) {
			$video = apms_query($video[0]);
			$video['content'] = preg_replace("/&amp;/", "&", $video['content']);
			$info = @parse_url($video['content']);
...
function apms_video($vid) {
	...
	if($video['type'] == "file") { //JWPLAYER
		...
		if($video['type'] == "youtube") { //유튜브
			...
			$show = '<iframe width="'.$video['width'].'" height="'.$video['height'].'" src="//www.youtube.com/embed/'.$video['vid'].'?autohide=1&vq=hd720&wmode=opaque'.$vlist.$autoplay.'" frameborder="0"'.$fullscreen.'></iframe>';
		} else if($video['type'] == "vimeo") { //비메오
			$autoplay = ($autoplayvideo || $video['auto']) ? '&amp;autoplay=1' : '';
			$show = '<iframe src="http://player.vimeo.com/video/'.$video['vid'].'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff'.$autoplay.'&amp;wmode=opaque" width="'.$video['width'].'" height="'.$video['height'].'" frameborder="0"'.$fullscreen.'></iframe>';
			...
			$show = '<iframe width="'.$video['width'].'" height="'.$video['height'].'" src="http://'.$video_url.$autoplay.'&wmode=opaque" frameborder="0" scrolling="no"'.$fullscreen.'></iframe>';
		} else if($video['type'] == "dailymotion") { //Dailymotion
			$show = '<iframe frameborder="0" width="'.$video['width'].'" height="'.$video['height'].'" src="http://www.dailymotion.com/embed/video/'.$video['vid'].'?&wmode=opaque"'.$fullscreen.'></iframe>';
			...
			$show = '<iframe src="//sendvid.com/embed/'.$video['vid'].'" width="'.$video['width'].'" height="'.$video['height'].'" frameborder="0"'.$fullscreen.'></iframe>';
...
function thema_switcher_load($type, $code) {
	...
	if(!isset($data['data_1']) || $data['data_1'] != THEMA) return;
...
function apms_editor_image($content, $mode='') {
	...
	for($i=0;$i<count($imgs[1]);$i++) {
		...
		if(is_file($destfile) && strpos($destfile, G5_DATA_DIR.'/'.G5_EDITOR_DIR)) {
...
function apms_widget_config($wid, $opt='', $mopt='', $addon=''){
	...
	if(isset($row['data_set']) && $row['data_set']) { 
...
function apms_board_rows($arr) {
	...
	if(!$bo_table || count($board_cnt) > 1 || $except) { //복수
		...
		$sql_common = "from {$g5['board_new_table']} where $sql_mode2 $sql_mode3 $sql_find $sql_term $sql_main $sql_image $sql_mb $sql_where";
		if($page > 1) {
		...
		$sca_query = "";
		if(isset($arr['ca_list']) && $arr['ca_list']) {
			$sca_query = (isset($arr['ex_ca']) && $arr['ex_ca']) ? "and find_in_set(ca_name, '{$arr['ca_list']}')=0" : "and find_in_set(ca_name, '{$arr['ca_list']}')";
		}
		...
		$tmp_write_table = $g5['write_prefix'] . $bo_table;
		$sql_common = "from $tmp_write_table where wr_is_comment = '{$sql_mode1}' $sql_mode3 $sca_query $sql_term $sql_main $sql_image $sql_mb $sql_where";
		if($page > 1) {
			$total = sql_fetch("select count(*) as cnt $sql_common ", false);
			$total_count = $total['cnt'];
			$total_page  = ceil($total_count / $rows);  // 전체 페이지 계산
...
function apms_member_rows($arr) {
	...
	//데모
	if(isset($demo_config['bo_table']) && $demo_config['bo_table']) $bo_list = $demo_config['bo_table'];
