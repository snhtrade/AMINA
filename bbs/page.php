...
if($is_content) {
	...
	$page_content .= conv_content($co_content, $co['co_html'], $co['co_tag_filter_use']);
	...
	if(IS_YC) {
		...
		$page_content = preg_replace($src, $dst, $page_content);
	}
	...
	if($at['content']) { //직접입력
		$page_content = apms_content($at['content'], false);
	} else {
		ob_start();
...
// SEO;
$seo_page_desc = apms_cut_text(preg_replace("!<style(.*?)<\/style>!is", "", $page_content), 200);
