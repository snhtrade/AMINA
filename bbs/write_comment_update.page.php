    ...
    if ($is_admin == 'super') // 최고관리자 통과
        ...
        if (chk_multiple_admin($member['mb_id'], $group['gr_admin'])) { // 자신이 관리하는 그룹인가?
            if ($member['mb_level'] >= $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
                ...
                apms_alert('1|그룹관리자의 권한보다 높은 회원의 댓글이므로 수정할 수 없습니다.');
        } else
            apms_alert('1|자신이 관리하는 그룹의 게시판이 아니므로 댓글을 수정할 수 없습니다.');
        ...
        if (chk_multiple_admin($member['mb_id'], $board['bo_admin'])) { // 자신이 관리하는 게시판인가?
            if ($member['mb_level'] >= $mb['mb_level']) // 자신의 레벨이 크거나 같다면 통과
                ...
                apms_alert('1|게시판관리자의 권한보다 높은 회원의 댓글이므로 수정할 수 없습니다.');
        } else
            apms_alert('1|자신이 관리하는 게시판이 아니므로 댓글을 수정할 수 없습니다.');
