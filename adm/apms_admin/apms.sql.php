...
sql_query(" ALTER TABLE `{$g5['apms_xp']}` ADD `editor_img` tinyint(4) NOT NULL DEFAULT '1' AFTER `item_cnt` ", false);
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	sql_query(" ALTER TABLE `{$g5['write_prefix']}{$row['bo_table']}` ADD `as_update` datetime NOT NULL default '0000-00-00 00:00:00' AFTER `as_thumb` ", false);
}
